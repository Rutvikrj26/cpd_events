...
class ZoomWebhookLog(BaseModel):
    ...
    webhook_id = models.CharField(
        max_length=100, unique=True, db_index=True, help_text="Zoom's webhook delivery ID (for deduplication)"
    )
    ...
    # Meeting/Event Info
    # =========================================
    zoom_meeting_id = models.CharField(max_length=20, db_index=True, help_text="Zoom meeting ID")
    zoom_meeting_uuid = models.CharField(max_length=100, blank=True, help_text="Zoom meeting UUID (unique per instance)")
    ...
    # Results
    attendance_records_created = models.PositiveIntegerField(default=0, help_text="Number of attendance records created")
    ...
    def is_processable(self):
        ...
        return self.processing_status in [self.ProcessingStatus.PENDING, self.ProcessingStatus.FAILED]
    ...
    def match_event(self):
        ...
        try:
            event = Event.objects.get(zoom_meeting_id=self.zoom_meeting_id, deleted_at__isnull=True)
            self.event = event
...
class ZoomRecording(BaseModel):
    ...
    event = models.ForeignKey('events.Event', on_delete=models.CASCADE, related_name='recordings')

    # Zoom Meeting Info
    zoom_meeting_id = models.CharField(max_length=20, db_index=True, help_text="Zoom meeting ID")
    zoom_meeting_uuid = models.CharField(max_length=100, help_text="Zoom meeting UUID (unique per instance)")
    zoom_recording_id = models.CharField(max_length=100, unique=True, db_index=True, help_text="Zoom's recording ID")

    topic = models.CharField(max_length=300, blank=True, help_text="Meeting topic from Zoom")
    ...
    zoom_expires_at = models.DateTimeField(null=True, blank=True, help_text="When recording expires on Zoom")

    # Access Control
    access_level = models.CharField(max_length=20, choices=AccessLevel.choices, default=AccessLevel.REGISTRANTS)
    zoom_password = models.CharField(max_length=50, blank=True, help_text="Password for Zoom share URL")
    access_password = models.CharField(max_length=50, blank=True, help_text="Optional platform password for access")

    # Display Settings
    title = models.CharField(max_length=200, blank=True, help_text="Display title (defaults to event title)")
    description = models.TextField(blank=True, max_length=2000, help_text="Description shown to viewers")
...
class ZoomRecordingFile(BaseModel):
    ...
    recording = models.ForeignKey(ZoomRecording, on_delete=models.CASCADE, related_name='files')

    zoom_file_id = models.CharField(max_length=100, help_text="Zoom's file ID")
    file_type = models.CharField(max_length=20, choices=FileType.choices)
    recording_type = models.CharField(
        max_length=50, blank=True, help_text="E.g., shared_screen_with_speaker_view, gallery_view"
    )
    ...
    # URLs
    download_url = models.URLField(max_length=2000, blank=True, help_text="Zoom download URL")
    play_url = models.URLField(max_length=2000, blank=True, help_text="Zoom play URL (for embedding)")
    share_url = models.URLField(max_length=2000, blank=True, help_text="Shareable URL")
    storage_url = models.URLField(max_length=500, blank=True, help_text="Our storage URL (if file is copied locally)")

    # Processing Status
    processing_status = models.CharField(max_length=20, default='completed', help_text="Zoom processing status")
    ...
    def is_downloadable(self):
        ...
        return self.file_type in [self.FileType.VIDEO, self.FileType.AUDIO, self.FileType.CHAT, self.FileType.TRANSCRIPT]
...
class RecordingView(BaseModel):
    ...
    recording = models.ForeignKey(ZoomRecording, on_delete=models.CASCADE, related_name='views')
    registration = models.ForeignKey('registrations.Registration', on_delete=models.CASCADE, related_name='recording_views')
