...
from common.fields import LowercaseEmailField, EncryptedTextField
...
class User(AbstractBaseUser, PermissionsMixin, SoftDeleteModel):
    ...
    profile_photo_url = models.URLField(
        blank=True,
        help_text="URL to profile photo"
    )
    ...
    notify_event_reminders = models.BooleanField(
        default=True,
        help_text="Receive event reminders"
    )
    notify_certificate_issued = models.BooleanField(
        default=True,
        help_text="Receive certificate notifications"
    )
    notify_marketing = models.BooleanField(
        default=False,
        help_text="Receive marketing emails"
    )
    notify_event_updates = models.BooleanField(
        default=True,
        help_text="Receive event change notifications"
    )
    ...
    organizer_linkedin = models.URLField(
        blank=True,
        help_text="LinkedIn profile URL"
    )
    organizer_twitter = models.CharField(
        max_length=100,
        blank=True,
    ...
    certificates_earned_count = models.PositiveIntegerField(
        default=0,
        help_text="Number of certificates earned"
    )
    total_cpd_credits = models.DecimalField(
    ...
    # Organizer stats
    events_hosted_count = models.PositiveIntegerField(
        default=0,
        help_text="Number of events hosted"
    )
...
class ZoomConnection(BaseModel):
    ...
    access_token = EncryptedTextField(
        help_text="Zoom OAuth access token"
    )
    refresh_token = EncryptedTextField(
        help_text="Zoom OAuth refresh token"
    )
    ...
    last_used_at = models.DateTimeField(
        null=True, blank=True,
        help_text="Last time connection was used"
    )
    ...
    def disconnect(self):
        ...
        self.refresh_token = ''
        self.save(update_fields=[
            'is_active', 'access_token', 'refresh_token', 'updated_at'
        ])
