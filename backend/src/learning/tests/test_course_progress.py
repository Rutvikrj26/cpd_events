...
class TestCourseProgressArchitecture:
    def test_course_enrollment_progress(self):
        ...
        # Setup
        user = make_user()
        course_owner = make_user("owner@example.com")
        Subscription.objects.update_or_create(
            user=course_owner,
            defaults={"plan": Subscription.Plan.LMS, "status": Subscription.Status.ACTIVE},
        )
        course = Course.objects.create(title="Test Course", slug="test-course", owner=course_owner)
    ...
    def test_validation_mutual_exclusivity(self):
        ...
        # Course context
        course_owner = make_user("course_owner@example.com")
        Subscription.objects.update_or_create(
            user=course_owner,
            defaults={"plan": Subscription.Plan.LMS, "status": Subscription.Status.ACTIVE},
        )
        course = Course.objects.create(title="Conflict Course", slug="conflict-course", owner=course_owner)
        enrollment = CourseEnrollment.objects.create(course=course, user=user)
