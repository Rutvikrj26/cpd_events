...
class EventModuleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Release Settings', {
            'fields': (
                'release_type', 'release_at', 
                'release_days_after_registration', 'prerequisite_module'
            )
        }),
        ('Scoring & Credits', {
            'fields': ('passing_score', 'cpd_credits', 'cpd_type')
        }),
        ('Status', {
            'fields': ('is_published',)
        }),
...
class ModuleProgressAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'module', 'get_user', 'status', 
        'contents_completed', 'contents_total', 'score'
    ]
    list_filter = ['status']
