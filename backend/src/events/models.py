...
class Event(SoftDeleteModel):
    ...
    registration_enabled = models.BooleanField(
        default=True,
        help_text="Accept new registrations"
    )
    ...
    registration_deadline = models.DateTimeField(
        null=True, blank=True,
        help_text="Registration cutoff time"
    )
    
    # Waitlist
    waitlist_enabled = models.BooleanField(
        default=False,
        help_text="Enable waitlist when full"
    )
    waitlist_max = models.PositiveIntegerField(
        null=True, blank=True,
        help_text="Max waitlist size"
    )
    
    # Guest registration
    allow_guest_registration = models.BooleanField(
        default=True,
        help_text="Allow registration without account"
    )
    ...
    zoom_join_url = models.URLField(
        blank=True,
        help_text="Zoom join URL for attendees"
    )
    zoom_start_url = models.URLField(
        max_length=2000,
        blank=True,
        help_text="Zoom start URL for host"
    )
    zoom_password = models.CharField(
        max_length=50,
        blank=True,
        help_text="Zoom meeting password"
    )
    ...
    cpd_enabled = models.BooleanField(
        default=True,
        help_text="Event offers CPD credits"
    )
    ...
    cpd_accreditation_note = models.TextField(
        blank=True,
        max_length=1000,
    ...
    auto_issue_certificates = models.BooleanField(
        default=False,
        help_text="Auto-issue when event completes"
    )
    ...
    recording_enabled = models.BooleanField(
        default=False,
        help_text="Enable cloud recording"
    )
    recording_auto_publish = models.BooleanField(
        default=False,
        help_text="Auto-publish recording when available"
    )
    ...
    is_public = models.BooleanField(
        default=True,
        help_text="Visible in public listings"
    )
    ...
    registration_count = models.PositiveIntegerField(
        default=0,
        help_text="Confirmed registrations"
    )
    waitlist_count = models.PositiveIntegerField(
        default=0,
        help_text="Waitlisted registrations"
    )
    attendance_count = models.PositiveIntegerField(
        default=0,
        help_text="Attendees who joined"
    )
    certificate_count = models.PositiveIntegerField(
        default=0,
        help_text="Certificates issued"
    )
    ...
    def can_waitlist(self):
        ...
        if not self.is_full:
            return False
        if self.waitlist_max and self.waitlist_count >= self.waitlist_max:
            return False
        return True
    ...
    def update_counts(self):
        ...
        self.registration_count = registrations.filter(
            status=Registration.Status.CONFIRMED
        ).count()
        
        self.waitlist_count = registrations.filter(
            status=Registration.Status.WAITLISTED
        ).count()
        
        self.attendance_count = registrations.filter(
            attended=True
        ).count()
        
        self.certificate_count = registrations.filter(
            certificate_issued=True
        ).count()
        
        self.save(update_fields=[
            'registration_count', 'waitlist_count',
            'attendance_count', 'certificate_count',
            'updated_at'
        ])
    ...
    def duplicate(self, new_title=None, new_start=None):
        ...
        new_event.slug = generate_unique_slug(
            new_event.title,
            set(Event.objects.values_list('slug', flat=True))
        )
