...
class EventSession(BaseModel):
    ...
    speaker_names = models.CharField(max_length=500, blank=True, help_text="Comma-separated list of speakers")
    ...
    # Status
    is_mandatory = models.BooleanField(default=True, help_text="Whether this session is required for certificate")
    ...
    @property
    def minimum_required_minutes(self):
        """Calculate the minimum required attendance minutes for this session."""
        return (self.duration_minutes * self.minimum_attendance_percent) / 100
...
class SessionAttendance(BaseModel):
    ...
    override_by = models.ForeignKey(
        'accounts.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='session_attendance_overrides'
    )
    ...
    def calculate_eligibility(self):
        ...
        self.is_eligible = self.duration_minutes >= self.session.minimum_required_minutes
