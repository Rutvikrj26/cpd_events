...
class TestSyncAttendanceEndpoint:
    ...
    def test_sync_attendance_async_mode(self, api_client, db):
        ...
        event = Event.objects.create(
            title="Async Event", owner=user, zoom_meeting_id="123456789", format="online", starts_at=timezone.now()
        )
    ...
    def test_sync_attendance_sync_mode(self, api_client, db):
        ...
        event = Event.objects.create(
            title="Sync Event", owner=user, zoom_meeting_id="123456789", format="online", starts_at=timezone.now()
        )
    ...
    def test_sync_attendance_no_zoom(self, api_client, db):
    ...
    def test_sync_attendance_pulls_from_zoom(self, api_client, db):
        ...
        reg = Registration.objects.create(
            event=event, full_name="Confirmed User", email="confirmed@example.com", status="confirmed"
        )
