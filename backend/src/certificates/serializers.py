...
from .models import Certificate, CertificateTemplate, CertificateStatusHistory
...
class CertificateTemplateListSerializer(SoftDeleteModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'uuid', 'name', 'version', 'is_default',
            'is_latest_version', 'use_count',
            'created_at',
        ]
...
class CertificateTemplateDetailSerializer(SoftDeleteModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'file_url', 'preview_url', 'field_positions',
            'dimensions', 'orientation',
            'is_default', 'is_latest_version', 'use_count',
            'original_template',
            'created_at', 'updated_at',
        ]
        read_only_fields = [
            'uuid', 'version', 'is_latest_version', 'use_count',
            'original_template', 'created_at', 'updated_at',
        ]
...
class CertificateTemplateCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'field_positions', 'dimensions', 'orientation', 'is_default',
        ]
...
class CertificateListSerializer(SoftDeleteModelSerializer):
    ...
    class Meta:
        model = Certificate
        fields = [
            'uuid', 'event_title', 'registrant_name',
            'status', 'short_code', 'issued_at',
            'created_at',
        ]
        read_only_fields = fields
...
class CertificateDetailSerializer(SoftDeleteModelSerializer):
    ...
    class Meta:
        model = Certificate
        fields = [
            'uuid', 'event', 'registration', 'template',
            'status', 'verification_code', 'short_code',
            'certificate_data', 'pdf_url', 'public_url',
            'issued_at', 'expires_at',
            'revocation_reason', 'revoked_at', 'revoked_by',
            'view_count', 'download_count', 'last_viewed_at',
            'verification_url',
            'created_at', 'updated_at',
        ]
        read_only_fields = fields
    ...
    def validate(self, attrs):
        if not attrs.get('registration_uuids') and not attrs.get('issue_all_eligible'):
            raise serializers.ValidationError(
                'Either registration_uuids or issue_all_eligible is required.'
            )
        return attrs
...
class PublicCertificateVerificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'status', 'is_valid',
            'event', 'registrant', 'organizer',
            'certificate_data', 'issued_at', 'expires_at',
        ]
...
class MyCertificateSerializer(SoftDeleteModelSerializer):
    ...
    def get_download_url(self, obj):
        if obj.status == 'issued' and obj.pdf_url:
            return obj.pdf_url  # TODO: Generate signed URL (M7)
        return None
