...
from common.permissions import CanCreateCourses, CanCreateEvents, CanIssueCertificates
...
class CertificateTemplateViewSet(SoftDeleteModelViewSet):
    ...
    @swagger_auto_schema(
        operation_summary="Get available templates",
        operation_description="""
        Returns all certificate templates available to the current user.

        User access logic:
        - Returns only user's own templates (organization sharing removed)
        """,
    )
    @action(detail=False, methods=["get"], url_path="available")
    def available_templates(self, request):
        """Get all templates available to the current user (own templates only)."""
        user = request.user

        templates = CertificateTemplate.objects.filter(owner=user, is_active=True, deleted_at__isnull=True)
