...
class SoftDeleteFilter(django_filters.FilterSet):
    ...
    include_deleted = django_filters.BooleanFilter(method='filter_include_deleted', label='Include deleted records')
...
class DateRangeFilter(django_filters.FilterSet):
    ...
    from_date = django_filters.DateFilter(field_name='created_at', lookup_expr='date__gte', label='From date')
    to_date = django_filters.DateFilter(field_name='created_at', lookup_expr='date__lte', label='To date')
