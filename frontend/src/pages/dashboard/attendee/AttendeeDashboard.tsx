  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Tabs defaultValue="overview" className="space-y-8">
        ...
        {/* Certificates Tab */}
        <TabsContent value="certificates" className="animate-in fade-in-50">
          <Card>
            <CardHeader>
              <CardTitle>My Certificates</CardTitle>
              <CardDescription>
                All certificates you have earned from completed events.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {registrations.filter(r => r.certificate_issued).length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground mt-2">
                    Certificates will appear here once you attend events and they are issued by the organizer.
                  </p>
                </div>
              ) : (
                  ...
                  <table className="w-full caption-bottom text-sm text-left">
                    <thead className="[&_tr]:border-b">
                      <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                        ...
                        <th className="h-12 px-4 align-middle font-medium text-muted-foreground">Date Issued</th>
                    ...
                    <tbody className="[&_tr:last-child]:border-0">
                      {registrations
                        .filter(r => r.certificate_issued)
                        .map((reg) => (
                          <tr key={reg.uuid} className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                            <td className="p-4 align-middle font-medium">
                              <Link to={`/events/${reg.event.slug || reg.event.uuid}`} className="hover:underline">
                                {reg.event.title}
                              </Link>
                            </td>
                            <td className="p-4 align-middle">
                              {new Date(reg.certificate_issued_at!).toLocaleDateString()}
                            </td>
                            <td className="p-4 align-middle">
                              <Badge variant="secondary">
                                {Number(reg.event.cpd_credit_value)} {reg.event.cpd_credit_type}
                              </Badge>
                            </td>
                            <td className="p-4 align-middle text-right">
                              <Button
                                ...
                                size="sm"
                                className="h-8 gap-2"
                                onClick={() => reg.certificate_url && window.open(reg.certificate_url, '_blank')}
                                disabled={!reg.certificate_url}
                              >
